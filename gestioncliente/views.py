...
def import_file_cliente(request):
    ...
    row_num = 0
    columns = ['Primer nombre Cliente','Segundo nombre Cliente',
               'Tercer nombre Cliente','Primer apellido Cliente','Primer apellido Cliente',
               'correo_electronico','celular','edad',
               'direccion_postal']
    ...
    for row in range(1):
        ...
        for col_num in range(9):
            if col_num == 0:
                ws.write(row_num, col_num, 'Primer nombre' , font_style)
            if col_num == 1:
                ws.write(row_num, col_num, 'Segundo nombre' , font_style)
            if col_num == 2:
                ws.write(row_num, col_num, 'Tercer nombre' , font_style)
            if col_num == 3:
                ws.write(row_num, col_num, 'Apellido1' , font_style)
            if col_num == 4:
                ws.write(row_num, col_num, 'Apellido2' , font_style)
            if col_num == 5:
                ws.write(row_num, col_num, 'correo_electronico' , font_style)
            if col_num == 6:
                ws.write(row_num, col_num, 'celular' , font_style)
            if col_num == 7:
                ws.write(row_num, col_num, 'edad' , font_style)
            if col_num == 8:
                ws.write(row_num, col_num, 'direccion_postal' , font_style)
    wb.save(response)
    return response  
...
def cliente_carga_masiva_save(request):
    ...
    if request.method == 'POST':
        ...
        for item in df.itertuples():
            ...
            nombre2 = str(item[2])  
            nombre3 = str(item[3])  
            apellido1 = str(item[4])  
            apellido2 = str(item[5])  
            correo_electronico = str(item[6])  
            celular = int(item[7])  
            edad = int(item[8])  
            direccion_postal = int(item[9])              
