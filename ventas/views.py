...
def crear_venta(request):
    if request.method == 'POST':
        ...
        venta.total = total
        venta.save()
        messages.add_message(request, messages.INFO, 'Se ha creado una venta!')
...
def listar_ventas(request):
    ...
    if busqueda:
        ventas = ventas.filter(cliente__nombre1__icontains=busqueda)
...
def agregar_item(request, venta_id):
    ...
    productos = Producto.objects.all()
    messages.add_message(request, messages.INFO, 'Se ha agregado un producto a su venta!')
...
def eliminar_item(request, venta_id, item_id):
    ...
    item.delete()
    messages.add_message(request, messages.INFO, 'Se ha eliminado un producto a su venta!')
    return redirect('ver_venta', venta_id=venta.id)
...
def eliminar_venta(request, venta_id):
    ...
    venta.delete()
    messages.add_message(request, messages.INFO, 'Se ha eliminado su venta!')
    return redirect('listar_ventas')
...
def generar_reporte_ventas(request):
    ...
    if filtro_nombre:
        ventas = ventas.filter(cliente__nombre1__icontains=filtro_nombre)
